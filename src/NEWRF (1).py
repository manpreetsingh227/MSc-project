...
def rf(final_segments, n_previous_splits=1, thresholds=np.linspace(0.01, 0.5, 40),
       random_state=seed, mode='oracle', epsilon=0.04):
    results = []
    ...
    first_split = final_segments[segment_keys[0]]
    last_accepted_data = (first_split[features], first_split["target"])

    for t in range(n_previous_splits, len(segment_keys)):
        current_key = segment_keys[t]

        if mode == "oracle":
            train_keys = segment_keys[t - n_previous_splits : t]
        ...
        X_test = final_segments[current_key][features]
        y_test = final_segments[current_key]["target"]

        prev_key = segment_keys[t - 1]
        prev_data = final_segments[prev_key][features]

        reference_data, _ = impute_features(prev_data, prev_data, features_with_missing)
        _, train_filled = impute_features(prev_data, X_train, features_with_missing)
        _, test_filled  = impute_features(prev_data, X_test,  features_with_missing)

        scaler = StandardScaler().fit(reference_data.values)
        ...
        tn = np.sum(accepted & (y_true == 0))
        fn = np.sum(accepted & (y_true == 1))
        utility = utility_fn(tn, fn)

        results.append({
            "segment": current_key,
            "segment_num": int(current_key.split("_")[1]),
        ...
        if mode != "oracle":
            last_accepted_data = (X_test.loc[accept_decisions], y_test.loc[accept_decisions])
...
results = rf(final_segments, mode=mode, epsilon=epsilon)

results['seed'] = seed
results['mode'] = mode
results['epsilon'] = epsilon if epsilon is not None else "None"

output_name = f"rf_{mode}"
